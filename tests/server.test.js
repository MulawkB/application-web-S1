...
describe('Server', () => {
  ...
  describe('GET /api/v1/whisper', () => {
    it('should return an empty array when theres no data', async () => {
      await restoreDb()
  ...
  describe('GET /api/v1/whisper/:id', () => {
    it('should return 404 when the whisper does not exist', async () => {
      const response = await supertest(app).get(`/api/v1/whisper/${inventedId}`)
      expect(response.status).toBe(404)
    })
    it('should return a wisper details', async () => {
  ...
  describe('PUT /api/v1/whisper/:id', () => {
    ...
    it('Should return a 404 when the whisper does not exist', async () => {
    ...
    it('Should return a 200 when the whisper is updated', async () => {
      const response = await supertest(app).put(`/api/v1/whisper/${existingId}`).send({ message: 'whispers updated' })
      expect(response.status).toBe(200)
      const storedWhisper = await getById(existingId)
      expect(storedWhisper).toStrictEqual({ id: existingId, message: 'whispers updated' })
    })
  })
